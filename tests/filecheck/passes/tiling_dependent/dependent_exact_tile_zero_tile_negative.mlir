...
builtin.module {
  func.func @static_zero_tile_factor_is_not_used(%out: memref<?xf32>) {
    ...
    %k1_nat = "dtensor.nat.const"() <{value = 0 : i32}> : () -> !dtensor.nat
...
// CHECK: %[[K4:[0-9]+]] = "dtensor.nat.const"() <{value = 4 : i32}> : () -> !dtensor.nat
// CHECK: %[[K0:[0-9]+]] = "dtensor.nat.const"() <{value = 0 : i32}> : () -> !dtensor.nat
// CHECK: %[[K:[0-9]+]] = "dtensor.nat.mul"(%[[K4]], %[[K0]]) : (!dtensor.nat, !dtensor.nat) -> !dtensor.nat
