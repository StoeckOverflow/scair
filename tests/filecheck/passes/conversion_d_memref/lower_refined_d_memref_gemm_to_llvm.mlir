...
builtin.module {
  func.func @main() -> i32 {
    ...
    %Aflat = d_memref.alloc : () -> !d_memref.memref<[%A_size], f32>
    %Bflat = d_memref.alloc : () -> !d_memref.memref<[%B_size], f32>
    %Cflat = d_memref.alloc : () -> !d_memref.memref<[%C_size], f32>

    %A = d_memref.reinterpret_cast %Aflat
      : !d_memref.memref<[%A_size], f32>
        to !d_memref.memref<[%n, %k], f32, offset: %c0, strides: [%k, 1]>
    %B = d_memref.reinterpret_cast %Bflat
      : !d_memref.memref<[%B_size], f32>
        to !d_memref.memref<[%k, %m], f32, offset: %c0, strides: [%m, 1]>
    %C = d_memref.reinterpret_cast %Cflat
      : !d_memref.memref<[%C_size], f32>
        to !d_memref.memref<[%n, %m], f32, offset: %c0, strides: [%m, 1]>
    ...
    d_affine.for %i = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%n) step 1 : index {
      d_affine.for %j = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%k) step 1 : index {
        d_memref.store %f1, %A[%i, %j] : f32, !d_memref.memref<[%n, %k], f32, offset: %c0, strides: [%k, 1]>
        d_affine.yield
      }
      d_affine.yield
    }
    ...
    d_affine.for %i = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%k) step 1 : index {
      d_affine.for %j = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%m) step 1 : index {
        d_memref.store %f1, %B[%i, %j] : f32, !d_memref.memref<[%k, %m], f32, offset: %c0, strides: [%m, 1]>
        d_affine.yield
      }
      d_affine.yield
    }
    ...
    d_affine.for %i = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%n) step 1 : index {
      d_affine.for %j = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%m) step 1 : index {
        %sum = d_affine.for %p = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%k) step 1 : index iter_args(%acc = %f0 : f32) {
          %a = d_memref.load %A[%i, %p] : !d_memref.memref<[%n, %k], f32, offset: %c0, strides: [%k, 1]> -> f32
          %b = d_memref.load %B[%p, %j] : !d_memref.memref<[%k, %m], f32, offset: %c0, strides: [%m, 1]> -> f32
        ...
        d_memref.store %sum, %C[%i, %j] : f32, !d_memref.memref<[%n, %m], f32, offset: %c0, strides: [%m, 1]>
        d_affine.yield
      }
      d_affine.yield
    }
    ...
    %checksum = d_affine.for %i = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%n) step 1 : index iter_args(%acc = %f0 : f32) {
      %inner = d_affine.for %j = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%m) step 1 : index iter_args(%acc2 = %acc : f32) {
        %v = d_memref.load %C[%i, %j] : !d_memref.memref<[%n, %m], f32, offset: %c0, strides: [%m, 1]> -> f32
    ...
    d_memref.dealloc %Aflat : !d_memref.memref<[%A_size], f32>
    d_memref.dealloc %Bflat : !d_memref.memref<[%B_size], f32>
    d_memref.dealloc %Cflat : !d_memref.memref<[%C_size], f32>
    func.return %ret : i32
  }
}
...
// CHECK: %[[A_P_ONE:[0-9]+]] = "llvm.mlir.constant"() <{value = 1}> : () -> i64
// CHECK: %[[A_P_STRIDE:[0-9]+]] = "llvm.mul"(%[[P]], %[[A_P_ONE]]) : (i64, i64) -> i64
...
// CHECK: %[[B_J_ONE:[0-9]+]] = "llvm.mlir.constant"() <{value = 1}> : () -> i64
// CHECK: %[[B_J_STRIDE:[0-9]+]] = "llvm.mul"(%{{[0-9]+}}, %[[B_J_ONE]]) : (i64, i64) -> i64
...
// CHECK: %[[C_J_ONE:[0-9]+]] = "llvm.mlir.constant"() <{value = 1}> : () -> i64
// CHECK: %[[C_J_STRIDE:[0-9]+]] = "llvm.mul"(%{{[0-9]+}}, %[[C_J_ONE]]) : (i64, i64) -> i64
