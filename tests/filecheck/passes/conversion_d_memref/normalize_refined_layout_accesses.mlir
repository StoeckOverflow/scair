...
builtin.module {
  func.func @normalize(%stride0 : index, %stride1 : index, %i : index, %j : index) -> f32 {
    %d0 = "arith.constant"() <{value = 256 : index}> : () -> index
    %d1 = "arith.constant"() <{value = 1024 : index}> : () -> index
    %flat = "arith.constant"() <{value = 262144 : index}> : () -> index
  ...
  func.func @normalize_store(%stride0 : index, %stride1 : index, %i : index, %j : index, %v : f32) {
    %d0 = "arith.constant"() <{value = 8 : index}> : () -> index
    %d1 = "arith.constant"() <{value = 16 : index}> : () -> index
    %flat = "arith.constant"() <{value = 128 : index}> : () -> index
    %buf = d_memref.alloc : () -> !d_memref.memref<[%flat], f32>
...
// CHECK-NEXT:    %4 = "arith.constant"() <{value = 256 : index}> : () -> index
// CHECK-NEXT:    %5 = "arith.constant"() <{value = 1024 : index}> : () -> index
// CHECK-NEXT:    %6 = "arith.constant"() <{value = 262144 : index}> : () -> index
