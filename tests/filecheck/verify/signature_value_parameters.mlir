...
func.func @ok(%n : !dtensor.nat,
              %buf : !d_memref.memref<[%n], f32>) {
  func.return
}

// CHECK-LABEL: func.func @ok(
// CHECK-SAME: %0: !dtensor.nat
...
"test.region"() ({
^bb0(%n : !dtensor.nat, %buf : !d_memref.memref<[%n], f32>):
  "test.ret"() : () -> ()
}) : () -> ()

// CHECK: "test.region"() ({
// CHECK-NEXT: ^bb{{[0-9]+}}(%{{[0-9]+}}: !dtensor.nat, %{{[0-9]+}}: !d_memref.memref<[%{{[0-9]+}}], f32>):

// -----

"test.region"() ({
^bb0(%buf : !d_memref.memref<[%n], f32>, %n : !dtensor.nat):
  "test.ret"() : () -> ()
}) : () -> ()
