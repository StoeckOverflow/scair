...
builtin.module {
  ...
  %v = "test.v"() : () -> f32
  d_memref.store %v, %buf[%o_i, %o_i] : f32, !d_memref.memref<[%m, %n], f32>
  %r = d_memref.load %buf[%o_i, %o_i] : !d_memref.memref<[%m, %n], f32> -> f32
  %d0 = d_memref.dim %buf, %z_i : !d_memref.memref<[%m, %n], f32> -> index
  %dx = d_memref.dim_exact %buf {axis = 0 : i32} : !d_memref.memref<[%m, %n], f32> -> !value<%m>
  %sv = d_memref.subview %buf[%z_i, %z_i][%m_i, %n_i][%o_i, %o_i] : !d_memref.memref<[%m, %n], f32> -> !d_memref.memref<[%m, %n], f32>
  ...
  "test.keep"(%r, %d0, %dx, %c, %vx, %mx) : (f32, index, !value<%m>, !d_memref.memref<[%m, %n], f32>, !d_memref.vector<%m, f32>, !d_memref.matrix<%m, %n, f32>) -> ()
  d_memref.dealloc %buf : !d_memref.memref<[%m, %n], f32>
}

// VERIFY: d_memref.dim %{{.*}}, %{{.*}} : !d_memref.memref<[%{{.*}}, %{{.*}}], f32> -> index
// VERIFY: d_memref.dim_exact %{{.*}} {axis = 0 : i32} : !d_memref.memref<[%{{.*}}, %{{.*}}], f32> -> !value<%{{.*}}>
