...
builtin.module {
  ...
  %b = "d_tensor.split_dim"(%a, %nt, %tn) <{dim = 1 : i32}>
    : (!d_tensor.tensor<[%m, %n, %p], f32>, !d_tensor.size, !d_tensor.size)
      -> !d_tensor.tensor<[%m, %nt, %tn, %p], f32>
  "test.keep"(%b) : (!d_tensor.tensor<[%m, %nt, %tn, %p], f32>) -> ()
}

// CHECK: "d_tensor.split_dim"
// CHECK-SAME: (!d_tensor.tensor<[%0, %1, %2], f32>, !d_tensor.size, !d_tensor.size) -> !d_tensor.tensor<[%0, %3, %4, %2], f32>
...
// CHECK: "d_tensor.split_dim"
// CHECK-SAME: (!d_tensor.tensor<[%0], f32>, !d_tensor.size, !d_tensor.size) -> !d_tensor.tensor<[%1, %2], f32>
...
builtin.module {
  %outer = "d_tensor.size.param"() : () -> !d_tensor.size
  %src = "test.src"() : () -> !d_tensor.tensor<[], f32>
  %bad = "d_tensor.split_dim"(%src, %outer, %outer) <{dim = 0 : i32}>
    : (!d_tensor.tensor<[], f32>, !d_tensor.size, !d_tensor.size)
