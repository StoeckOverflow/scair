...
final class TensorOpsSpec extends AnyFlatSpec:
  ...
  "tensor type parser/printer" should "round-trip tensor/vector/matrix with SSA dims" in {
    val ctx = MLContext()
  ...
  "tensor.add" should "reject shape mismatch when dims are semantically equal but not SSA-identical" in {
  ...
  "tensor.dim" should "return the embedded dim SSA value for the selected axis" in {
    val d0 = Value[Attribute](TensorNatType())
    val d1 = Value[Attribute](TensorNatType())
    val tensor = Value[TensorTensorType](
      TensorTensorType(Seq(ValueAttribute(d0), ValueAttribute(d1)), Float32Type())
    )
    val dim = Dim(tensor, IntegerAttr(IntData(1), I32), Result(TensorNatType()))
