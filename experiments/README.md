...
|---|---|---|---|---|---|
| `matmul_outer_dim_tiling_benchmark` | Core thesis evidence | `MATMUL_OUTER_DIM_TILING_ITERATIONS=1 experiments/matmul_outer_dim_tiling_benchmark/build_matmul_outer_dim_tiling_example.sh` | structural `metrics.csv`, `metrics.json`, `summary.md`, per-route IR/LLVM/runtime artifacts | primary apples-to-apples MLIR comparison: all routes tile the same `i/j` output loops; dependent `M0*M1` and `N0*N1` proofs remove tails | Runtime checksum/result plus structural tail counts |
| `matmul_reduction_dim_tiling_benchmark` | Supporting validation | `BENCH_WARMUP_REPS=0 BENCH_TIMING_REPS=1 MATMUL_REDUCTION_DIM_TILING_ITERATIONS=1 MATMUL_REDUCTION_DIM_TILING_SIZE_SET=128x128x12x64 experiments/matmul_reduction_dim_tiling_benchmark/build_matmul_reduction_dim_tiling_example.sh` | common-schema `metrics.csv`, `summary.md`, IR/LLVM/runtime artifacts | focused proof benchmark for `K=K0*K1`; dependent route removes reduction tail guards, but this is not a full matmul tiling comparison | Runtime checksum/result |
| `matmul_full_factorized_tiling_benchmark` | Supporting validation | `MATMUL_FULL_FACTORIZED_TILING_ITERATIONS=1 experiments/matmul_full_factorized_tiling_benchmark/build_matmul_full_factorized_tiling_example.sh` | structural `metrics.csv`, `metrics.json`, `summary.md`, per-route IR/LLVM/runtime artifacts | composition evidence: `M/N/K` product proofs compose for full factorized tiling; reduction tiling is not itself a parallel speedup claim | Runtime checksum/result plus structural tail counts |
...
| `conv2d_output_dim_tiling_benchmark` | Core thesis evidence | `CONV2D_OUTPUT_DIM_TILING_ITERATIONS=1 experiments/conv2d_output_dim_tiling_benchmark/build_conv2d_output_dim_tiling_example.sh` | structural `metrics.csv`, `metrics.json`, `summary.md`, per-route IR | primary fair Conv2D tiling comparison: all routes tile `n/cout/oh/ow`; dependent product proofs remove output-tail guards | Structural validation |
| `conv2d_reduction_dim_tiling_benchmark` | Supporting validation | `CONV2D_REDUCTION_DIM_TILING_ITERATIONS=1 experiments/conv2d_reduction_dim_tiling_benchmark/build_conv2d_reduction_dim_tiling_example.sh` | structural `metrics.csv`, `metrics.json`, `summary.md`, per-route IR | supporting Conv2D reduction evidence: tiles `Cin * Kh * Kw` by `Cin1 * Kh * Kw`; not a full Conv2D scheduling claim | Structural validation |
| `conv2d_full_factorized_tiling_benchmark` | Supporting validation | `CONV2D_FULL_FACTORIZED_TILING_ITERATIONS=1 experiments/conv2d_full_factorized_tiling_benchmark/build_conv2d_full_factorized_tiling_example.sh` | structural `metrics.csv`, `metrics.json`, `summary.md`, per-route IR | composition evidence that output and reduction product proofs can remove provable guards in one Conv2D loop nest | Structural validation |
...
| `build_all_metrics.sh` | Aggregate runner | `MATMUL_REDUCTION_DIM_TILING_ITERATIONS=1 MATMUL_OUTER_DIM_TILING_ITERATIONS=1 MATMUL_FULL_FACTORIZED_TILING_ITERATIONS=1 ITERATIONS=1 experiments/build_all_metrics.sh` | `experiments/out/all_metrics.csv`, `summary.md`, env snapshot, `structural_metrics_manifest.json`, `structural/` | concatenates common-schema benchmark families and archives structural tiling evidence separately | Depends on family |
...
- `tiling_correctness_matrix`, `conv2d_output_dim_tiling_benchmark`,
  `conv2d_reduction_dim_tiling_benchmark`,
  `conv2d_full_factorized_tiling_benchmark`,
  `matmul_outer_dim_tiling_benchmark`, and
  `matmul_full_factorized_tiling_benchmark`: tiling route, proof, loop-step,
  and tail-bound operation counts.
