builtin.module {
  func.func @conv2d_dynamic(
    ...
    "func.call"(%n_nat, %cin_nat, %h_nat, %w_nat, %cout_nat, %kh_nat, %kw_nat, %oh_nat, %ow_nat,
      ...
      : (!d_tensor.nat, !d_tensor.nat, !d_tensor.nat, !d_tensor.nat, !d_tensor.nat, !d_tensor.nat, !d_tensor.nat, !d_tensor.nat, !d_tensor.nat,
         ...
         !d_memref.memref<[%n_nat, %cin_nat, %oh_nat, %ow_nat, %kh_nat, %kw_nat], f32, offset: 0, strides: [%chw, %hw, %w, %c1, %w, %c1]>,
         !d_memref.memref<[%cout_nat, %cin_nat, %kh_nat, %kw_nat], f32, offset: 0, strides: [%cin_khkw, %khkw, %kw, %c1]>,
  ...
  func.func @conv2d_dynamic_typed(
    ...
    %y_stride1 : index,
    %y_stride2 : index,
    %X : !d_memref.memref<[%n_nat, %cin_nat, %oh_nat, %ow_nat, %kh_nat, %kw_nat],
         f32, offset: 0, strides: [%x_stride0, %x_stride1, %x_stride2, %unit_stride, %x_stride2, %unit_stride]>,
    %K : !d_memref.memref<[%cout_nat, %cin_nat, %kh_nat, %kw_nat],
         f32, offset: 0, strides: [%k_stride0, %k_stride1, %k_stride2, %unit_stride]>,
    %Y : !d_memref.memref<[%n_nat, %cout_nat, %oh_nat, %ow_nat],
      ...
      d_affine.for %co = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%cout) step 1 : index {
        d_affine.for %oh_idx = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%oh) step 1 : index {
          d_affine.for %ow_idx = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%ow) step 1 : index {
            %sum_ci = d_affine.for %ci = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%cin) step 1 : index iter_args(%acc_ci = %f0 : f32) {
              %sum_kh = d_affine.for %kh_idx = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%kh) step 1 : index iter_args(%acc_kh = %acc_ci : f32) {
                %sum_kw = d_affine.for %kw_idx = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%kw) step 1 : index iter_args(%acc_kw = %acc_kh : f32) {
                  %x = d_memref.load %X[%n_idx, %ci, %oh_idx, %ow_idx, %kh_idx, %kw_idx] : !d_memref.memref<[%n_nat, %cin_nat, %oh_nat, %ow_nat, %kh_nat, %kw_nat], f32, offset: 0, strides: [%x_stride0, %x_stride1, %x_stride2, %unit_stride, %x_stride2, %unit_stride]> -> f32
                  %k = d_memref.load %K[%co, %ci, %kh_idx, %kw_idx] : !d_memref.memref<[%cout_nat, %cin_nat, %kh_nat, %kw_nat], f32, offset: 0, strides: [%k_stride0, %k_stride1, %k_stride2, %unit_stride]> -> f32
            ...
            d_memref.store %sum_ci, %Y[%n_idx, %co, %oh_idx, %ow_idx] : f32, !d_memref.memref<[%n_nat, %cout_nat, %oh_nat, %ow_nat], f32, offset: 0, strides: [%y_stride0, %y_stride1, %y_stride2, %unit_stride]>
            d_affine.yield
          }
          d_affine.yield
        }
