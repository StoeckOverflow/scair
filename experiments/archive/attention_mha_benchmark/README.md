...
Archived supporting experiment. This benchmark is preserved because it shows that
`H = heads * head_dim` provenance can guide exact hidden-dimension tiling in an MHA-shaped kernel.
It is not part of the active thesis-facing aggregate suite because the MLIR baseline does not tile
the same loop target, so it is not a clean apples-to-apples comparison.
...
- `value_dependent_exact_tile`: ScaIR dependent route with an exact tile over the flattened hidden
  dimension
- `value_dependent`: compatibility route name; thesis-facing exact-tiling runs should use
  `value_dependent_exact_tile`
...
- exact-tiling claim: ScaIR derives a dynamic step from `hidden = heads * head_dim` provenance for
  ...
  without that proof
- positivity claim: the value-dependent route treats `heads` and `head_dim` as builtin `index`
  dimensions and relies on the same explicit positive-step validation used by the matmul and
...
- The value-dependent route runs `validate-d-affine-dynamic-steps` before lowering. Legality comes
  from explicit index product provenance and positive dynamic-step validation.
- The separate product-loop factorization pass rewrites a flat product loop into two unit-step
  factor loops and should not be used as same-step tiling evidence.
- `loop_transform`, `tile_loop`, `shared_tile_size`, `tile_size_source`, `dynamic_step_present`,
  `tail_cleanup_present`, and `exact_divisibility_proof` are route notes derived from build-script
...
- `affine_cleanup_present`, `factorized_tile_count`, and `tail_free_factorized` are legacy artifact
  ...
  index product provenance, not from these regex notes alone.
