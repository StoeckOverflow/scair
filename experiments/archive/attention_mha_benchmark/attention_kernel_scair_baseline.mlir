builtin.module {
  ...
  func.func @attention_mha(
    ...
    %Q = "memref.reinterpret_cast"(%Qflat, %c0, %batch, %seq, %heads, %head_dim, %seq_hidden, %hidden, %head_dim, %c1)
      <{operandSegmentSizes = array<i32: 1, 1, 4, 4>}>
      : (memref<?xf32>, index, index, index, index, index, index, index, index, index)
        -> memref<?x?x?x?xf32, strided<[?, ?, ?, ?], offset: 0>>

    %K = "memref.reinterpret_cast"(%Kflat, %c0, %batch, %seq, %heads, %head_dim, %seq_hidden, %hidden, %head_dim, %c1)
      <{operandSegmentSizes = array<i32: 1, 1, 4, 4>}>
      : (memref<?xf32>, index, index, index, index, index, index, index, index, index)
        -> memref<?x?x?x?xf32, strided<[?, ?, ?, ?], offset: 0>>

    %V = "memref.reinterpret_cast"(%Vflat, %c0, %batch, %seq, %heads, %head_dim, %seq_hidden, %hidden, %head_dim, %c1)
      <{operandSegmentSizes = array<i32: 1, 1, 4, 4>}>
      : (memref<?xf32>, index, index, index, index, index, index, index, index, index)
        -> memref<?x?x?x?xf32, strided<[?, ?, ?, ?], offset: 0>>
    ...
    %tmpOut = "memref.reinterpret_cast"(%tmpOutFlat, %c0, %batch, %seq, %heads, %head_dim, %seq_hidden, %hidden, %head_dim, %c1)
      <{operandSegmentSizes = array<i32: 1, 1, 4, 4>}>
      : (memref<?xf32>, index, index, index, index, index, index, index, index, index)
        -> memref<?x?x?x?xf32, strided<[?, ?, ?, ?], offset: 0>>

    %out = "memref.reinterpret_cast"(%outFlat, %c0, %batch, %seq, %heads, %head_dim, %seq_hidden, %hidden, %head_dim, %c1)
      <{operandSegmentSizes = array<i32: 1, 1, 4, 4>}>
      : (memref<?xf32>, index, index, index, index, index, index, index, index, index)
        -> memref<?x?x?x?xf32, strided<[?, ?, ?, ?], offset: 0>>
        ...
        affine.for %i = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%seq) step 1 : index {
          affine.for %j = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%seq) step 1 : index {
            %sum = affine.for %d = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%head_dim) step 1 : index iter_args(%acc = %f0 : f32) {
              %qv = "memref.load"(%Q, %b, %i, %h, %d) : (memref<?x?x?x?xf32, strided<[?, ?, ?, ?], offset: 0>>, index, index, index, index) -> f32
              %kv = "memref.load"(%K, %b, %j, %h, %d) : (memref<?x?x?x?xf32, strided<[?, ?, ?, ?], offset: 0>>, index, index, index, index) -> f32
            ...
            "memref.store"(%scaled, %score, %b, %h, %i, %j) : (f32, memref<?x?x?x?xf32, strided<[?, ?, ?, ?], offset: 0>>, index, index, index, index) -> ()
          }
        }
      }
    }
      ...
      affine.for %h = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%heads) step 1 : index {
        affine.for %i = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%seq) step 1 : index {
          %row_max = affine.for %j = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%seq) step 1 : index iter_args(%acc = %fneg : f32) {
            %sv = "memref.load"(%score, %b, %h, %i, %j) : (memref<?x?x?x?xf32, strided<[?, ?, ?, ?], offset: 0>>, index, index, index, index) -> f32
          ...
          %denom = affine.for %j = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%seq) step 1 : index iter_args(%acc = %f0 : f32) {
            ...
            "memref.store"(%expv, %prob, %b, %h, %i, %j) : (f32, memref<?x?x?x?xf32, strided<[?, ?, ?, ?], offset: 0>>, index, index, index, index) -> ()
            %next = "arith.addf"(%acc, %expv) <{fastmath = #arith.fastmath<none>}> : (f32, f32) -> f32
            affine.yield %next : f32
          }
          affine.for %j = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%seq) step 1 : index {
            %expv = "memref.load"(%prob, %b, %h, %i, %j) : (memref<?x?x?x?xf32, strided<[?, ?, ?, ?], offset: 0>>, index, index, index, index) -> f32
            %norm = "arith.divf"(%expv, %denom) <{fastmath = #arith.fastmath<none>}> : (f32, f32) -> f32
            "memref.store"(%norm, %prob, %b, %h, %i, %j) : (f32, memref<?x?x?x?xf32, strided<[?, ?, ?, ?], offset: 0>>, index, index, index, index) -> ()
          }
        }
      }
    }
        ...
        affine.for %h = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%heads) step 1 : index {
          affine.for %d = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%head_dim) step 1 : index {
            %sum = affine.for %j = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%seq) step 1 : index iter_args(%acc = %f0 : f32) {
              %pv = "memref.load"(%prob, %b, %h, %i, %j) : (memref<?x?x?x?xf32, strided<[?, ?, ?, ?], offset: 0>>, index, index, index, index) -> f32
              %vv = "memref.load"(%V, %b, %j, %h, %d) : (memref<?x?x?x?xf32, strided<[?, ?, ?, ?], offset: 0>>, index, index, index, index) -> f32
              %prod = "arith.mulf"(%pv, %vv) <{fastmath = #arith.fastmath<none>}> : (f32, f32) -> f32
              %next = "arith.addf"(%acc, %prod) <{fastmath = #arith.fastmath<none>}> : (f32, f32) -> f32
              affine.yield %next : f32
            }
            "memref.store"(%sum, %tmpOut, %b, %i, %h, %d) : (f32, memref<?x?x?x?xf32, strided<[?, ?, ?, ?], offset: 0>>, index, index, index, index) -> ()
          }
        }
        affine.for %h_copy = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%heads) step 1 : index {
          affine.for %d_copy = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%head_dim) step 1 : index {
            %v = "memref.load"(%tmpOut, %b, %i, %h_copy, %d_copy) : (memref<?x?x?x?xf32, strided<[?, ?, ?, ?], offset: 0>>, index, index, index, index) -> f32
            "memref.store"(%v, %out, %b, %i, %h_copy, %d_copy) : (f32, memref<?x?x?x?xf32, strided<[?, ?, ?, ?], offset: 0>>, index, index, index, index) -> ()
          }
        }
      }
    }
