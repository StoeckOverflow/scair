builtin.module {
  ...
  func.func @attention_mha(
    ...
    %Q = d_memref.reinterpret_cast %Qflat
      : !d_memref.memref<[], f32>
        to !d_memref.memref<[%batch, %seq, %hidden], f32,
    ...
    %score = d_memref.reinterpret_cast %scoreFlat
      : !d_memref.memref<[], f32>
        to !d_memref.memref<[%batch, %heads, %seq, %seq], f32,
        ...
        d_affine.for %i = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%seq) step 1 : index {
          d_affine.for %j = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%seq) step 1 : index {
            %sum = d_affine.for %d = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%head_dim) step 1 : index iter_args(%acc = %f0 : f32) {
              ...
              %qv = d_memref.load %Q[%b, %i, %hd] : !d_memref.memref<[%batch, %seq, %hidden], f32, offset: 0, strides: [%seq_hidden, %hidden, %c1]> -> f32
              %kv = d_memref.load %K[%b, %j, %hd] : !d_memref.memref<[%batch, %seq, %hidden], f32, offset: 0, strides: [%seq_hidden, %hidden, %c1]> -> f32
            ...
            d_memref.store %scaled, %score[%b, %h, %i, %j] : f32, !d_memref.memref<[%batch, %heads, %seq, %seq], f32, offset: 0, strides: [%heads_seq_seq, %seq_seq, %seq, %c1]>
            d_affine.yield
          }
          d_affine.yield
        }
      ...
      d_affine.for %h = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%heads) step 1 : index {
        d_affine.for %i = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%seq) step 1 : index {
          %row_max = d_affine.for %j = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%seq) step 1 : index iter_args(%acc = %fneg : f32) {
            %sv = d_memref.load %score[%b, %h, %i, %j] : !d_memref.memref<[%batch, %heads, %seq, %seq], f32, offset: 0, strides: [%heads_seq_seq, %seq_seq, %seq, %c1]> -> f32
          ...
          %denom = d_affine.for %j = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%seq) step 1 : index iter_args(%acc = %f0 : f32) {
            ...
            d_memref.store %expv, %prob[%b, %h, %i, %j] : f32, !d_memref.memref<[%batch, %heads, %seq, %seq], f32, offset: 0, strides: [%heads_seq_seq, %seq_seq, %seq, %c1]>
            %next = "arith.addf"(%acc, %expv) <{fastmath = #arith.fastmath<none>}> : (f32, f32) -> f32
            d_affine.yield %next : (f32)
          }
          d_affine.for %j = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%seq) step 1 : index {
            %expv = d_memref.load %prob[%b, %h, %i, %j] : !d_memref.memref<[%batch, %heads, %seq, %seq], f32, offset: 0, strides: [%heads_seq_seq, %seq_seq, %seq, %c1]> -> f32
            %norm = "arith.divf"(%expv, %denom) <{fastmath = #arith.fastmath<none>}> : (f32, f32) -> f32
            d_memref.store %norm, %prob[%b, %h, %i, %j] : f32, !d_memref.memref<[%batch, %heads, %seq, %seq], f32, offset: 0, strides: [%heads_seq_seq, %seq_seq, %seq, %c1]>
            d_affine.yield
          }
          d_affine.yield
        }
        ...
        d_affine.for %h = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%heads) step 1 : index {
          ...
          d_affine.for %d = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%head_dim) step 1 : index {
            ...
            %sum = d_affine.for %j = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%seq) step 1 : index iter_args(%acc = %f0 : f32) {
              %pv = d_memref.load %prob[%b, %h, %i, %j] : !d_memref.memref<[%batch, %heads, %seq, %seq], f32, offset: 0, strides: [%heads_seq_seq, %seq_seq, %seq, %c1]> -> f32
              %vv = d_memref.load %V[%b, %j, %hd] : !d_memref.memref<[%batch, %seq, %hidden], f32, offset: 0, strides: [%seq_hidden, %hidden, %c1]> -> f32
              %prod = "arith.mulf"(%pv, %vv) <{fastmath = #arith.fastmath<none>}> : (f32, f32) -> f32
              %next = "arith.addf"(%acc, %prod) <{fastmath = #arith.fastmath<none>}> : (f32, f32) -> f32
              d_affine.yield %next : (f32)
            }
            d_memref.store %sum, %tmpOut[%b, %i, %hd] : f32, !d_memref.memref<[%batch, %seq, %hidden], f32, offset: 0, strides: [%seq_hidden, %hidden, %c1]>
            d_affine.yield
          }
          d_affine.yield
        }
        %copy_hidden = d_affine.for %hd = affine_map<(d0) -> (d0)>(%c0) to affine_map<(d0) -> (d0)>(%hidden) step 1 : index iter_args(%acc_seed = %f0 : f32) {
          %v = d_memref.load %tmpOut[%b, %i, %hd] : !d_memref.memref<[%batch, %seq, %hidden], f32, offset: 0, strides: [%seq_hidden, %hidden, %c1]> -> f32
          d_memref.store %v, %out[%b, %i, %hd] : f32, !d_memref.memref<[%batch, %seq, %hidden], f32, offset: 0, strides: [%seq_hidden, %hidden, %c1]>
