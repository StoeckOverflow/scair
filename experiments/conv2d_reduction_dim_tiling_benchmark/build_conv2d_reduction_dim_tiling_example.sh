...
EXAMPLE_DIR="$ROOT/experiments/conv2d_reduction_dim_tiling_benchmark"
...
for dims in "${sizes[@]}"; do
  ...
  if route_enabled "mlir_baseline_reduction_tile"; then
    ...
    append_row "mlir_baseline_reduction_tile" "$dims" "$tile" "$MLIR_SRC" "$out" "mlir-opt --affine-loop-tile=tile-size=$tile" "arith.muli_index" "upstream_reference_may_tile_only_legal_outer_bands"
  fi
  if route_enabled "ordinary_scair_reduction_tile_with_tail"; then
    ...
    append_row "ordinary_scair_reduction_tile_with_tail" "$dims" "$tile" "$ORDINARY_SRC" "$out" "$pipeline" "arith.muli_index" "ordinary_tiles_by_Cin1KhKw_but_keeps_tail"
  fi
  if route_enabled "dependent_reduction_guarded_tail_simplified"; then
    ...
    reject_tail "$out"
    append_row "dependent_reduction_guarded_tail_simplified" "$dims" "dynamic_Cin1KhKw" "$DEPENDENT_SRC" "$out" "$pipeline" "dtensor.nat.mul" "guarded_artifact=$(basename "$guarded");proof_removes_reduction_tail"
  fi
  if route_enabled "dependent_reduction_exact_tile"; then
    ...
    run_scair "$DEPENDENT_SRC" "$pipeline" "$out"
    reject_tail "$out"
    append_row "dependent_reduction_exact_tile" "$dims" "dynamic_Cin1KhKw" "$DEPENDENT_SRC" "$out" "$pipeline" "dtensor.nat.mul" "diagnostic_direct_exact_reduction_tiling"
  fi
done
