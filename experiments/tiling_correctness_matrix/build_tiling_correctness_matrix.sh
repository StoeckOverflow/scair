...
EXAMPLE_DIR="$ROOT/experiments/tiling_correctness_matrix"
...
| `dependent_exact_dynamic` | `dependent_exact_dynamic` | Explicit `dtensor.nat.mul` with `!dtensor.posnat` factor exact-tiles with dynamic step and no tail. |
...
[
  ...
  },
  {
    ...
    "script_route": "refine-positive-nats-from-asserts,canonicalize-dtensor-nat-products,dependent-exact-tile,dependent-tail-min-simplify,validate-d-affine-dynamic-steps,lower-refined-control-flow-to-llvm,lower-cf-assert-to-llvm,erase-dtensor-nat-proofs-to-index",
    "expected_tail": "none",
  ...
  },
  {
    ...
    "script_route": "canonicalize-dtensor-nat-products,dependent-product-loop-exact-tile",
...
validate_case() {
  ...
  case "$case_name" in
    ordinary_tail)
      ...
      reject_ir_pattern "$path" 'dtensor\.nat\.mul|d_affine\.for' "ordinary route must not use dependent product proof or d_affine loop"
      ;;
    non_divisible_ordinary)
      ...
      reject_ir_pattern "$path" 'dtensor\.nat\.mul|d_affine\.for' "non-divisible ordinary route must not use dependent product proof or d_affine loop"
      ;;
    dependent_exact_dynamic)
      require_ir_pattern "$path" 'dtensor\.nat\.mul' "dependent dynamic route must preserve natmul proof before erasure"
    ...
    runtime_checked_dynamic)
      ...
      reject_ir_pattern "$path" 'dtensor\.|d_affine\.for|cf\.assert|arith\.minsi| to min |affine\.min|d_affine\.min' "runtime checked route must erase proofs and tails"
      ;;
    zero_negative)
      require_ir_pattern "$path" 'dtensor\.nat\.const.*value = 0' "zero negative control must keep explicit zero factor"
    ...
    nested_commuted_product)
      require_ir_pattern "$path" 'dtensor\.nat\.mul' "nested/commuted route must preserve explicit product proof before erasure"
    ...
    nested_commuted_product_lazy)
      require_ir_pattern "$path" 'dtensor\.nat\.mul' "lazy nested/commuted route must preserve explicit product proof before erasure"
    ...
    tail_product_factor_lazy)
      require_ir_pattern "$path" 'dtensor\.nat\.mul' "tail product-factor route must preserve explicit product proof before erasure"
...
run_case "dependent_exact_dynamic" \
  "dependent_exact_dynamic" \
  "canonicalize-dtensor-nat-products,dependent-product-loop-exact-tile,validate-d-affine-dynamic-steps" \
  "positive_exact" \
  "K0*K1" \
  "posnat_type" \
  "none" \
...
run_case "dependent_static_affine" \
  ...
  "canonicalize-dtensor-nat-products,dependent-product-loop-exact-tile,d-affine-to-affine-compatible,canonicalize,cse,dce" \
...
run_case "runtime_checked_dynamic" \
  ...
  "refine-positive-nats-from-asserts,canonicalize-dtensor-nat-products,dependent-exact-tile,dependent-tail-min-simplify,validate-d-affine-dynamic-steps,lower-refined-control-flow-to-llvm,lower-cf-assert-to-llvm,erase-dtensor-nat-proofs-to-index,canonicalize,cse,dce" \
...
run_case "zero_negative" \
  ...
  "canonicalize-dtensor-nat-products,dependent-product-loop-exact-tile" \
  "negative_control" \
  "const4*const0" \
