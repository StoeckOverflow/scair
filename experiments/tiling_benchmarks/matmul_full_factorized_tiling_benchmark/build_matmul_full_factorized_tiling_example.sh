...
for dims in "${sizes[@]}"; do
  ...
  if route_enabled "dependent_full_guarded_tail_simplified"; then
    ...
    run_scair_opt -s "$DEPENDENT_SRC" --passes "canonicalize,cse,dce,canonicalize-d-tensor-size-products,dependent-context-band-factor-tile-with-tail,dependent-tile-with-tail-control,validate-d-affine-dynamic-steps,canonicalize,cse,dce" > "$guarded"
    ...
    build_scair_route \
      ...
      "$DEPENDENT_SRC" \
      "$DEPENDENT_DRIVER" \
      "canonicalize,cse,dce,canonicalize-d-tensor-size-products,dependent-context-band-factor-tile-with-tail,dependent-tile-with-tail-control,dependent-tail-min-simplify,validate-d-affine-dynamic-steps,canonicalize,cse,dce" \
      "canonicalize,cse,dce,canonicalize-d-tensor-size-products,dependent-context-band-factor-tile-with-tail,dependent-tile-with-tail-control,dependent-tail-min-simplify,validate-d-affine-dynamic-steps,canonicalize,cse,dce,lower-d-memref-to-llvm" \
      "$tag" "$m0" "$m1" "$n0" "$n1" "$k0" "$k1"
    ...
    append_case "dependent_full_guarded_tail_simplified" "$tag" "$size" "d_tensor.size.mul" "no" "guarded_artifact=$(basename "$guarded");proof_removes_i_j_p_tail"
  fi
  ...
  if route_enabled "dependent_full_exact_tile"; then
    ...
    build_scair_route \
      ...
      "$DEPENDENT_SRC" \
      "$DEPENDENT_DRIVER" \
      "canonicalize,cse,dce,canonicalize-d-tensor-size-products,dependent-context-band-exact-tile,dependent-product-loop-exact-tile,validate-d-affine-dynamic-steps,canonicalize,cse,dce" \
      "canonicalize,cse,dce,canonicalize-d-tensor-size-products,dependent-context-band-exact-tile,dependent-product-loop-exact-tile,validate-d-affine-dynamic-steps,canonicalize,cse,dce,lower-d-memref-to-llvm" \
      "$tag" "$m0" "$m1" "$n0" "$n1" "$k0" "$k1"
    require_no_tail "$OUT_DIR/${tag}_dependent_full_exact_tile.tiled.mlir" "dependent exact route must be tail-free"
    append_case "dependent_full_exact_tile" "$tag" "$size" "d_tensor.size.mul" "no" "diagnostic_direct_exact_i_j_p_tiling"
  fi
done
