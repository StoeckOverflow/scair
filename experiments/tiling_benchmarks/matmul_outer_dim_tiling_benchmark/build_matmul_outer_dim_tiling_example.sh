...
for dims in "${sizes[@]}"; do
  ...
  if route_enabled "dependent_mn_guarded_tail_simplified"; then
    ...
    build_scair_route \
      ...
      "$DEPENDENT_SRC" \
      "$DEPENDENT_DRIVER" \
      "canonicalize,cse,dce,canonicalize-d-tensor-shape-products,dependent-context-band-factor-tile-with-tail,dependent-tail-min-simplify,validate-d-affine-dynamic-steps,canonicalize,cse,dce" \
  ...
  if route_enabled "dependent_mn_exact_tile"; then
    ...
    build_scair_route \
      ...
      "$DEPENDENT_SRC" \
      "$DEPENDENT_DRIVER" \
      "canonicalize,cse,dce,canonicalize-d-tensor-shape-products,dependent-context-band-exact-tile,validate-d-affine-dynamic-steps,canonicalize,cse,dce" \
      "canonicalize,cse,dce,canonicalize-d-tensor-shape-products,dependent-context-band-exact-tile,validate-d-affine-dynamic-steps,canonicalize,cse,dce,lower-d-memref-to-llvm" \
      "$tag" "$m0" "$m1" "$n0" "$n1" "$k"
    require_no_tail "$OUT_DIR/${tag}_dependent_mn_exact_tile.tiled.mlir" "dependent exact route must be tail-free"
    append_case "dependent_mn_exact_tile" "$tag" "$size" "arith.muli" "no" "diagnostic_direct_exact_i_j_tiling"
  fi
done
...
\`arith.muli\` facts for \`M=M0*M1\` and \`N=N0*N1\` to derive exact dynamic
