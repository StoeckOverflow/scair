...
BENCHMARK_NAME="matmul_outer_dim_tiling"
COMPILER_FLAGS="-O2"

MLIR_SRC="$EXAMPLE_DIR/matmul_outer_dim_mlir_baseline.mlir"
ORDINARY_SRC="$EXAMPLE_DIR/matmul_outer_dim_scair_ordinary.mlir"
DEPENDENT_SRC="$EXAMPLE_DIR/matmul_outer_dim_scair_value_dependent.mlir"
...
route_enabled() {
  local route="$1"
  local entry
  IFS=',' read -r -a route_list <<<"$MATMUL_OUTER_DIM_ROUTES"
...
size_descriptor() {
  ...
  local n1="$4"
  local k="$5"
  echo "m0=$m0;m1=$m1;m=$((m0 * m1));n0=$n0;n1=$n1;n=$((n0 * n1));k=$k"
}
...
compile_executable() {
  ...
  "$CC" -O2 \
    ...
    -DMATMUL_OUTER_DIM_M0="$m0" \
    -DMATMUL_OUTER_DIM_M1="$m1" \
    -DMATMUL_OUTER_DIM_N0="$n0" \
    -DMATMUL_OUTER_DIM_N1="$n1" \
    -DMATMUL_OUTER_DIM_K="$k" \
    "$driver" "$obj" -o "$exe"
}
...
build_mlir_route() {
  ...
  compile_executable "$variant" "$MLIR_DRIVER" "$prefix.ll" "$prefix" "$m0" "$m1" "$n0" "$n1" "$k"
  end_ns="$(now_ns)"
...
build_scair_route() {
  ...
  "$MLIR_TRANSLATE" --mlir-to-llvmir "$prefix.llvm.mlir" > "$prefix.ll"
  compile_executable "$variant" "$driver" "$prefix.ll" "$prefix" "$m0" "$m1" "$n0" "$n1" "$k"
  end_ns="$(now_ns)"
  run_benchmark_repeated "$prefix.output.txt" "$prefix.exec" "$ITERATIONS"
  printf 'build_status=ok\ncompile_ms=%s\n' "$(format_ms "$start_ns" "$end_ns")" >> "$prefix.output.txt"
}
...
cat > "$SUMMARY_MD" <<'MD'
# Matmul Outer-Dimension Tiling Benchmark Summary
...
append_case() {
  ...
  printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
    ...
    "i+j" "$MATMUL_OUTER_DIM_TILE_SIZE" "$product_representation" "$tail" "yes" "$mlir_baseline" \
...
IFS=',' read -r -a sizes <<<"$MATMUL_OUTER_DIM_SIZE_SET"
for dims in "${sizes[@]}"; do
  IFS='x' read -r m0 m1 n0 n1 k <<<"$dims"
  if [[ -z "${m0:-}" || -z "${m1:-}" || -z "${n0:-}" || -z "${n1:-}" || -z "${k:-}" ]]; then
    echo "error: invalid MATMUL_OUTER_DIM_SIZE_SET entry '$dims' (expected M0xM1xN0xN1xK)" >&2
    exit 1
  fi
  tag="$(size_tag "$dims")"
  size="$(size_descriptor "$m0" "$m1" "$n0" "$n1" "$k")"

  if route_enabled "mlir_baseline_mn_tile"; then
    echo "==> Building MLIR baseline M/N tile for $size"
    build_mlir_route "mlir_baseline_mn_tile" "$tag" "$m0" "$m1" "$n0" "$n1" "$k"
    require_pattern "$OUT_DIR/${tag}_mlir_baseline_mn_tile.tiled.mlir" 'step 64' "MLIR baseline must tile with step 64"
    require_pattern "$OUT_DIR/${tag}_mlir_baseline_mn_tile.tiled.mlir" ' to min ' "MLIR baseline must keep guarded min tails"
    append_case "mlir_baseline_mn_tile" "$tag" "$size" "arith.muli_index" "yes" "upstream_mlir_affine_loop_tile_same_i_j_loop_target"
  fi

  if route_enabled "ordinary_scair_mn_tile_with_tail"; then
    echo "==> Building ordinary ScaIR M/N tile with tail for $size"
    build_scair_to_mlir_lower_route \
      "ordinary_scair_mn_tile_with_tail" \
      "$ORDINARY_SRC" \
      "canonicalize,cse,dce,ordinary-affine-context-band-tile-with-tail:$MATMUL_OUTER_DIM_TILE_SIZE,canonicalize,cse,dce" \
      "$tag" "$m0" "$m1" "$n0" "$n1" "$k"
    require_pattern "$OUT_DIR/${tag}_ordinary_scair_mn_tile_with_tail.tiled.mlir" 'step 64' "ordinary ScaIR must tile with step 64"
    require_pattern "$OUT_DIR/${tag}_ordinary_scair_mn_tile_with_tail.tiled.mlir" ' to min ' "ordinary ScaIR must keep guarded min tails"
    append_case "ordinary_scair_mn_tile_with_tail" "$tag" "$size" "arith.muli_index" "no" "ordinary_scair_control_same_i_j_loop_target"
  fi
  ...
  if route_enabled "dependent_mn_guarded_tail_simplified"; then
    ...
    run_scair_opt -s "$DEPENDENT_SRC" --passes "canonicalize,cse,dce,canonicalize-d-tensor-size-products,dependent-context-band-factor-tile-with-tail,validate-d-affine-dynamic-steps,canonicalize,cse,dce" > "$guarded"
    ...
    build_scair_route \
      ...
      "$DEPENDENT_SRC" \
      "$DEPENDENT_DRIVER" \
      "canonicalize,cse,dce,canonicalize-d-tensor-size-products,dependent-context-band-factor-tile-with-tail,dependent-tail-min-simplify,validate-d-affine-dynamic-steps,canonicalize,cse,dce" \
      "canonicalize,cse,dce,canonicalize-d-tensor-size-products,dependent-context-band-factor-tile-with-tail,dependent-tail-min-simplify,validate-d-affine-dynamic-steps,canonicalize,cse,dce,lower-d-memref-to-llvm" \
      "$tag" "$m0" "$m1" "$n0" "$n1" "$k"
    ...
    append_case "dependent_mn_guarded_tail_simplified" "$tag" "$size" "d_tensor.size.mul" "no" "guarded_artifact=$(basename "$guarded");proof_removes_i_j_tail"
  fi
  ...
  if route_enabled "dependent_mn_separable_tile"; then
    ...
    build_scair_route \
      ...
      "$DEPENDENT_SRC" \
      "$DEPENDENT_DRIVER" \
      "canonicalize,cse,dce,canonicalize-d-tensor-size-products,dependent-context-band-separable-tile,validate-d-affine-dynamic-steps,canonicalize,cse,dce" \
      "canonicalize,cse,dce,canonicalize-d-tensor-size-products,dependent-context-band-separable-tile,d-affine-to-affine-compatible,validate-d-affine-dynamic-steps,canonicalize,cse,dce,lower-d-memref-to-llvm" \
      "$tag" "$m0" "$m1" "$n0" "$n1" "$k"
    ...
    append_case "dependent_mn_separable_tile" "$tag" "$size" "d_tensor.size.mul" "no" "tiling_decision=separable;proof_source=size_product+affine_set;full_tile_branch_exact;partial_branch_guarded"
  fi

  if route_enabled "dependent_mn_exact_tile"; then
    echo "==> Building dependent exact M/N tile for $size"
    build_scair_route \
      "dependent_mn_exact_tile" \
      "$DEPENDENT_SRC" \
      "$DEPENDENT_DRIVER" \
      "canonicalize,cse,dce,canonicalize-d-tensor-size-products,dependent-context-band-exact-tile,validate-d-affine-dynamic-steps,canonicalize,cse,dce" \
      "canonicalize,cse,dce,canonicalize-d-tensor-size-products,dependent-context-band-exact-tile,validate-d-affine-dynamic-steps,canonicalize,cse,dce,lower-d-memref-to-llvm" \
      "$tag" "$m0" "$m1" "$n0" "$n1" "$k"
    require_no_tail "$OUT_DIR/${tag}_dependent_mn_exact_tile.tiled.mlir" "dependent exact route must be tail-free"
    append_case "dependent_mn_exact_tile" "$tag" "$size" "d_tensor.size.mul" "no" "diagnostic_direct_exact_i_j_tiling"
  fi
done
...
This benchmark is the thesis-facing apples-to-apples matmul tiling comparison.
...
\`d_tensor.size.mul\` facts for \`M=M0*M1\` and \`N=N0*N1\` to derive exact dynamic
...
echo
echo "Matmul outer-dimension tiling benchmark complete."
echo "Produced:"
