...
extern void _mlir_ciface_matmul_outer_dim_tiling(
    int64_t m0, int64_t m1, int64_t n0, int64_t n1, int64_t k,
...
int main(int argc, char **argv) {
  ...
  for (int64_t iter = 0; iter < iterations; ++iter) {
    fill(C, kCElements, 0.0f);
    _mlir_ciface_matmul_outer_dim_tiling(kM0, kM1, kN0, kN1, kK, &Aref, &Bref, &Cref);
  }
