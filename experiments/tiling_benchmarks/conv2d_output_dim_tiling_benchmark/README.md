...
This is the primary fair Conv2D tiling comparison. Upstream MLIR, ordinary ScaIR, and dependent
ScaIR all tile the same output-space loops: `n`, `cout`, `oh`, and `ow`. The dependent route uses
product facts for `N`, `Cout`, `OH`, and `OW` to remove conservative output-tail guards.
...
All routes target the same output-space loops. These loops are directly relevant to later parallel
scheduling because different output tiles write disjoint `Y` regions.

## What Is Not Claimed

This benchmark does not claim full Conv2D runtime superiority, cache tuning, vectorization, or
...
Key variables: `CONV2D_OUTPUT_DIM_TILING_SIZE_SET`, `CONV2D_OUTPUT_DIM_TILE_SIZE`,
`CONV2D_OUTPUT_DIM_ROUTES`.
