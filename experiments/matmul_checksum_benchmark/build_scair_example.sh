...
MATMUL_MLIR_SRC="${MATMUL_MLIR_SRC:-$EXAMPLE_DIR/matmul_kernel.mlir}"
...
require_file "$MATMUL_MLIR_SRC"
require_file "$CHECKSUM_MLIR_SRC"
require_file "$MLIR_DRIVER_SRC"
require_file "$MATMUL_BASELINE_SRC"
...
build_scair_kernel "lower-dynamic-memref-to-llvm-baseline" "$MATMUL_BASELINE_SRC" "$OUT_DIR/matmul_baseline_kernel_only_scair.o" "$OUT_DIR/matmul_baseline_kernel_only_scair.ll"
...
baseline_end=$(now_ns)
run_with_metrics "$OUT_DIR/matmul_baseline_kernel_only_scair_exec" "$OUT_DIR/matmul_baseline_kernel_only_scair_output.txt"
...
value_end=$(now_ns)
run_with_metrics "$OUT_DIR/matmul_value_dependent_scair_exec" "$OUT_DIR/matmul_value_dependent_scair_output.txt"
VALUE_COMPILE_MS="$(format_ms "$value_start" "$value_end")"

SUMMARY_MD="$OUT_DIR/summary.md"
SUMMARY_CSV="$OUT_DIR/metrics.csv"
write_summary_header "$SUMMARY_MD" "Matmul Checksum Benchmark Summary"
write_metrics_csv_header "$SUMMARY_CSV"

append_pair_row \
  "$SUMMARY_CSV" "$SUMMARY_MD" \
  "mlir_baseline" "mlir_baseline" \
  "$MATMUL_MLIR_SRC" "$CHECKSUM_MLIR_SRC" \
...
append_pair_row \
  ...
  "$OUT_DIR/matmul_baseline_kernel_only_scair_output.txt" \
  "$BASELINE_COMPILE_MS" \
...
append_pair_row \
  ...
  "$MATMUL_VALUE_DEP_SRC" "$CHECKSUM_VALUE_DEP_SRC" \
  "$OUT_DIR/matmul_value_dependent_scair.llvm.mlir" "$OUT_DIR/checksum_value_dependent_scair.llvm.mlir" \
  "$OUT_DIR/matmul_value_dependent_scair.ll" "$OUT_DIR/checksum_value_dependent_scair.ll" \
  "$OUT_DIR/matmul_value_dependent_scair_output.txt" \
  "$VALUE_COMPILE_MS" \
