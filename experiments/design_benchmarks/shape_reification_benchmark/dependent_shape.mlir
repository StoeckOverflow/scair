builtin.module {
  func.func @dependent_same_shape_different_ssa(
    ...
    %d0_size = "builtin.unrealized_conversion_cast"(%d0) : (!value<%m>) -> !d_tensor.size
    %e0_size = "builtin.unrealized_conversion_cast"(%e0) : (!value<%m>) -> !d_tensor.size
    %f0_size = "builtin.unrealized_conversion_cast"(%f0) : (!value<%m>) -> !d_tensor.size
    ...
    %d1_size = "builtin.unrealized_conversion_cast"(%d1) : (!value<%n>) -> !d_tensor.size
    %e1_size = "builtin.unrealized_conversion_cast"(%e1) : (!value<%n>) -> !d_tensor.size
    %f1_size = "builtin.unrealized_conversion_cast"(%f1) : (!value<%n>) -> !d_tensor.size


    %size_a = "arith.muli"(%a0_size, %a1_size) : (index, index) -> index
    %size_b = "arith.muli"(%b0_size, %b1_size) : (index, index) -> index
    %size_c = "arith.muli"(%c0_size, %c1_size) : (index, index) -> index
    %size_d = "arith.muli"(%d0_size, %d1_size) : (index, index) -> index
    %size_e = "arith.muli"(%e0_size, %e1_size) : (index, index) -> index
    %size_f = "arith.muli"(%f0_size, %f1_size) : (index, index) -> index
