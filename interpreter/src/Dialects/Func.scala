...
private def runFunctionWithArgs(
  ...
  if args.length != expectedIn.length then
    throw new Exception(
      s"$callSite: arity mismatch, expected ${expectedIn.length} args but got ${args.length}"
    )
  ...
  if entry.arguments.length != expectedIn.length then
    throw new Exception(
      s"$callSite: callee @${callee.sym_name.stringLiteral} entry block has ${entry.arguments.length} args but function type expects ${expectedIn.length}"
    )
  ...
  for op <- entry.operations do
    interpreter.interpret_op(op, new_ctx)
...
object run_call extends OpImpl[func.Call]:
  ...
  def compute(
    ...
    if op.callee.rootRef.stringLiteral == "print" then
      ...
      val callee = interpreter.symbolTable
        .get(calleeName)
        .getOrElse(
        ...
        case other        =>
          throw new Exception(
            s"func.call: symbol @$calleeName does not resolve to func.func (got ${other.name})"
          )
...
object run_call_indirect extends OpImpl[func.CallIndirect]:
  ...
  def compute(
    ...
    val handleVal = args.headOption.getOrElse(
    ...
    if handle.functionType != op.callee.typ then
      throw new Exception(
        s"func.call_indirect: type mismatch, handle type ${handle.functionType} does not match callee operand type ${op.callee.typ}"
      )
    ...
    val callee = interpreter.symbolTable
      ...
      case other        =>
        throw new Exception(
          s"func.call_indirect: symbol @${handle.symbol} does not resolve to func.func (got ${other.name})"
        )

    if callee.function_type != handle.functionType then
      throw new Exception(
        s"func.call_indirect: type mismatch, handle type ${handle.functionType} does not match target function type ${callee.function_type}"
      )
