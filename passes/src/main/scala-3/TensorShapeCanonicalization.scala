package scair.passes.tensor_shape_canonicalize
...
private def constValue(v: Value[TensorNatType]): Option[(BigInt, IntegerType | IndexType)] =
  v.owner match
...
private def mkNatConst(
  ...
  NatConst(IntegerAttr(IntData(k), typ), Result(TensorNatType()))

private val NatAddFold = pattern {
  case NatAdd(lhs, rhs, _) =>
...
private val NatMulFold = pattern {
  case NatMul(lhs, rhs, _) =>
    (constValue(lhs), constValue(rhs)) match
...
final class TensorShapeCanonicalize(ctx: MLContext) extends WalkerPass(ctx):
